...
export function encryptedTokenSerializer(
  secret: string,
  scheme: EncryptionMethods | EncryptionScheme = 'aes-256-gcm'
): TokenSerializer {
  const encryptor = makeCustomEncryptor(secret, scheme);
  ...
  return {
    stringify<Data = unknown>(data: OtpData<Data>): string {
      ...
      const parts = encryptor.encrypt(JSON.stringify(values));
      return parts.join('.');
    },
    parse<Data = unknown>(token: string): OtpData<Data> {
      ...
      try {
        parts = JSON.parse(
          encryptor.decrypt(majorParts[0], majorParts[1], majorParts[2])
        );
      } catch {
        throw new OtpError('BAD_REQUEST', 'BAD_TOKEN');
      }
      ...
      console.log('major parts', majorParts);

      const customData = parts.length == 5 ? parts[5] : undefined;
