...
export function openTokenEncryptedDataSerializer(
  secret: string,
  ...
  return {
    stringify<Data = unknown>(data: OtpData<Data>): string {
      ...
      const parts = [
        encodeBase64Url(utf8Encoder.encode(JSON.stringify(values))),
      ];
      const hasCustomData =
        data.customData !== null && data.customData !== undefined;

      if (hasCustomData) {
        const [iv, tag, encryptedData] = encryptor.encrypt(
    ...
    parse<Data = unknown>(token: string): OtpData<Data> {
      ...
      const customData =
        majorParts.length == 4
          ? JSON.parse(
              encryptor.decrypt(majorParts[1], majorParts[2], majorParts[3])
            )
          : undefined;
